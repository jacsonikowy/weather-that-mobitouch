...
import { setCityInModal, setModalActive } from 'features/cityInModal/cityInModal';
    ...
    return (
        <div className={styles.favorites}>
            ...
            <div className={styles.content}>
                {favorites.length !== 0 ?
               <table className={styles.favoritePanels}>
                        {favorites.map(city => {
                            console.log(city)
                            return <FavoritePanel name={city.name} weatherImg={city.weather[0].icon} pressure={city.main.pressure} temp={city.main.temp} weatherData={city} onClick={() =>{
                             dispatch(setCityInModal(city))   
                             dispatch(setModalActive(true))
                            }}/>
                        })}

                </table> : <span>asdas</span>
                }
            </div>
